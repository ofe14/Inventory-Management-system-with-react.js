...
const AllShelves = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'shelves'), (snapshot) => {
      const shelvesData = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      setAllShelves(shelvesData);
      setFiltered(shelvesData);
    });

    return () => unsubscribe();
  }, []);

  const gotoShelf = async (name) => {
    navigate(`/admin/shelves/one/${encodeURIComponent(name)}`);
  };
  ...
  const onSearchChangeHandler = (e) => {
    ...
    const filteredShelves = allShelves.filter((shelf) =>
      shelf.name.toLowerCase().includes(searchQuery)
    );
    setFiltered(filteredShelves);
  };
